...
// import { uploadStudioMedia } from "@/lib/studio/mediaStorage";
...
export async function POST(request: Request) {
  ...
  try {
    const uploadResult = await uploadStudioMedia({
      fileName,
    ...
    if (process.env.NODE_ENV === "development") {
      console.warn("Studio media upload failed.", error);
    }

    return NextResponse.json(
      {
        error: "Unable to store media file.",
        message: "Studio media upload failed.",
      },
      {
        status: 500,
      },
